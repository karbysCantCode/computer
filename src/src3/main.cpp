...
int main(int argc, char* argv[]) {
  CLIOptions options;
  const auto exeDir = FileHelper::getExecutableDirectory();
  const auto cachedArchPath = exeDir / cachedArchFileName;
  options.newArchPath = cachedArchPath;
  ...
  if (options.spasm) {
    std::cout << "Currently unsupported, please use smake.\n";
  }
